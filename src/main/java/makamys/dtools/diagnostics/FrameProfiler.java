...
import static makamys.dtools.diagnostics.FrameProfiler.Entry.*;
...
public class FrameProfiler implements IModEventListener {
    ...
    public FrameProfiler() {
        DToolsCommand.registerSubCommand("frameprofiler", new FrameProfilerSubCommand());
    }
    ...
    private boolean dumpProfilingResults() {
    	try {
    		tb.writeToCSV(Util.childFile(DTools.OUT_DIR, "frameprofiler.csv"));
    		return true;
    	} catch(IOException e) {
    		return false;
    	} finally {
    ...
    private static class FrameProfilerSubCommand implements ISubCommand {
        ...
        public void processCommand(ICommandSender sender, String[] args) {
            String usage = "dtools frameprofiler <start|stop|help>";
            if(args.length == 2) {
                switch(args[1]) {
                    case "start": {
