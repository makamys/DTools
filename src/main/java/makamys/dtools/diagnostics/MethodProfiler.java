...
import static makamys.dtools.command.DToolsCommand.*;
...
public class MethodProfiler {
    ...
    public void init() {
        ...
        DToolsCommand.registerSubCommand("methodprofiler", new MethodProfilerSubCommand());
    ...
    public boolean start(ProfileMode mode) {
        try {
            out = new FileWriter(Util.childFile(DTools.OUT_DIR, "method-profiler-trace.log"));
            profileMode = mode;
            return true;
        } catch (IOException e) {
    ...
    private static class MethodProfilerSubCommand implements ISubCommand {
        ...
        public void processCommand(ICommandSender sender, String[] args) {
            String usage = "dtools methodprofiler <start|stop|help>";
            if(args.length >= 2) {
                switch(args[1]) {
                    case "start": {
                        if(MethodProfiler.instance.isStarted()) {
                            ...
                            if(args.length == 3) {
                                ...
                                throw new WrongUsageException("dtools methodprofiler start <mode>", new Object[0]);
                            }
                        }
                        return;
                    }
