...
import static makamys.dtails.command.DTailsCommand.*;
import static makamys.dtails.diagnostics.FrameProfiler.Entry.*;
...
import makamys.dtails.util.TableBuilder;
import makamys.dtails.util.Util;
...
public class FrameProfiler implements IModEventListener {
    ...
    public FrameProfiler() {
        DTailsCommand.registerSubCommand("frameprofiler", new FrameProfilerSubCommand());
    }
    ...
    private boolean dumpProfilingResults() {
    	try {
    		tb.writeToCSV(Util.childFile(DTails.OUT_DIR, "frameprofiler.csv"));
    		return true;
    	} catch(IOException e) {
    		return false;
    	} finally {
    ...
    private static class FrameProfilerSubCommand implements ISubCommand {
        ...
        public void processCommand(ICommandSender sender, String[] args) {
            String usage = "dtails frameprofiler <start|stop|help>";
            if(args.length == 2) {
                switch(args[1]) {
                    case "start": {
                    ...
                    case "help": {
                        ...
                        addColoredChatMessage(sender, "The report will be written to " + HELP_EMPHASIS_COLOR + "./dtails/out/frameprofiler.csv" + HELP_COLOR + ".", HELP_COLOR);
                        addColoredChatMessage(sender, "A useful script for parsing the results of the report can be found at " + HELP_EMPHASIS_COLOR + "https://github.com/makamys/DTails/tree/master/scripts", HELP_COLOR, (msg) -> msg.getChatStyle().setChatClickEvent(new ClickEvent(Action.OPEN_URL, "https://github.com/makamys/DTails/tree/master/scripts")));
                        return;
                    }
                }
            }
